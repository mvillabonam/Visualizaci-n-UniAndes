...
# Se puede observar que se tiene información del PIB y de los niveles de CO2 desde 1960 hasta 2024


# Total de países (únicos)
n_distinct(data_pib$country_name)
n_distinct(data_co2$country_name)
# Para un total de 266 paises 


# Pasamos a formato long y se miran las estadisticas descrpitivas
pib_long <- data_pib %>%
...
# Valores extremos para cada tabla
# PIB
pib_min <- pib_long %>%
...
# Observamos como fue el comportamiento general por año
pib_anual <- pib_long %>%
  group_by(anio) %>%
...
plot_pib <- ggplot(pib_anual, aes(x = anio, y = promedio_pib)) +
  ...
  labs(
    title = "Promedio del PIB per cápita por año",
    x = "Año",
    y = "PIB per cápita"
  ) +
  theme_minimal()
...
plot_co2 <- ggplot(co2_anual, aes(x = anio, y = promedio_co2)) +
  ...
  labs(
    title = "Promedio de las emisiones de dioxido de carbono per cápita por año",
    x = "Año",
...
# Exportar
ggsave(filename = file.path(fig_path, "Primera_comparacion_pib_co2.png"),
...
# Ahora especificamente las emisiones de los paises con promedio del pib mas alto y mas bajo
...
# Algo interesante es que los dos paises con mayo pib (Monca y Liechtenstein) no cuentan con reporte de emisiones de C02
paises_mayor_pib <- pib_promedio_pais %>%
  slice_head(n = 3) %>%
  pull(country_name)

# Tomar los 5 países con menor PIB promedio
paises_menor_pib <- pib_promedio_pais %>%
  slice_tail(n = 3) %>%
  pull(country_name)

#Filtramos sobre el df de co2
...
comparacion_emisiones <- ggplot(co2_filtrado, aes(x = anio, y = valor_co2, color = country_name)) +
  ...
  geom_point(size = 1.8) +
  labs(
    title = "Evolución de las emisiones per cápita",
    subtitle = "Comparación de las emisiones historicas entre los 3 países con mayor y menor PIB promedio",
    x = "Año",
...
# Exportar
ggsave(filename = file.path(fig_path, "Evolucion de las emisiones.png"),
       plot = comparacion_emisiones, width = 16, height = 6, dpi = 300)
