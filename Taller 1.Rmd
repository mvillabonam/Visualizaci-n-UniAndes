...
map_data <- col_mpios_key %>%
  left_join(
    agg_departamento_key %>%
      select(dp, densidad_1985, densidad_2018, crec_abs, crec_pct),
    by = c("dpto" = "dp")
  ) %>%
  mutate(
    # reemplaza Inf/NaN por NA
    crec_pct = replace(crec_pct, !is.finite(crec_pct), NA_real_)
  )
