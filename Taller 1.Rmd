...
- **pob_censal**: dpnom, mpio  son tipo caracter mientras que dp, poblacion, dpmp, ano son tipo numérico (64 bits). 
  ...
  que diferencian cada registro entre si, mientras que poblacion hace referencia a una variable numérica de medición
...
```
```{r Joins, echo=FALSE, message=FALSE}
# --- población
outer_joint <- full_join( mun_area, pob_censal,by = c("dpmp", "dp", "ano", "llave"))
left_join_result <- left_join(pob_censal, mun_area, by = c("dpmp", "dp", "ano", "llave"))
```
...
5. Detecte datos faltantes y decida su tratamiento, si es necesario en el contexto de este
ejercicio. 
- **Respuesta**: En las bases de datos originales no hay valores vacíos que sea necesario tratar. 
  Por otra parte, tomando la base 
...
pob_censal <- pob_censal %>%
  mutate(
    dpnom = tools::toTitleCase(tolower(dpnom)),
    mpio  = tools::toTitleCase(tolower(mpio))
  )
sort(unique(pob_censal$mpio))
sort(unique(pob_censal$dpnom))
```
