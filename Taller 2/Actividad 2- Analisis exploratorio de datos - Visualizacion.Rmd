...
resumen_dificultades <- dificultades_emprend %>%
  count(respuesta, name = "conteo") %>%
  arrange(desc(conteo))

print(resumen_dificultades)

# --- 4. Gráfico comparativo
ggplot(resumen_dificultades, aes(x = respuesta, y = conteo, fill = respuesta)) +
  geom_col() +
  labs(title = "Dificultades para emprender en hogares sin productos financieros",
...
```
```{r Observacion de Variables relacionadas con el acceso al sistema financiero}
total_hogares_sin_fin <- length(unique(hogares_sin_finanzas))

resumen_dificultades_prop <- resumen_dificultades %>%
  mutate(proporcion = conteo / total_hogares_sin_fin * 100)

print(resumen_dificultades_prop)

# --- Gráfico con proporciones
ggplot(resumen_dificultades_prop, aes(x = respuesta, y = proporcion, fill = respuesta)) +
  geom_col() +
  labs(title = "Dificultades para emprender en hogares sin productos financieros",
       x = "Tipo de dificultad",
